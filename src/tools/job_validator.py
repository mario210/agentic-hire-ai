...
class ExpirationCheck(BaseModel):
    is_active: bool = Field(description="True if the job posting is currently active and accepting applications. False if it is expired, closed, not found, or filled.")
    reason: str = Field(description="A short explanation of why the job is considered active or expired.")
...
class JobValidator:
    ...
    def is_job_valid(self, job: JobOffer) -> Tuple[bool, str]:
        ...
        try:
            ...
            if not result.is_active:
                logger.info(f"Job '{job.title}' is expired/inactive. Reason: {result.reason}")
    ...
    def filter_valid_jobs(self, jobs: List[JobOffer]) -> List[JobOffer]:
        logger.info("🔍 Validating found job URLs to ensure they are active and not expired...")
        valid_jobs = []
        for job in jobs:
            ...
            if is_valid:
                ...
                logger.info(f"❌ Dropping dead or expired job link: [{job.url}] {title} - {reason}")
        return valid_jobs
